    ...
    <b-container>
      <b-row>
        ...
        <b-col class="product-part" md="10" sm="9">
          <b-row>
            <div class="empty-product" v-if="product.length==0 || !product.length">Không có sản phẩm</div>
            <ProductItem v-for="item in product" :key="item.productId" :data="item"></ProductItem>
          </b-row>
        </b-col>
      </b-row>
      <notifications 
...
export default {
  ...
  methods:{
    ...mapActions(['getProducts','getBrandFilter','filterByBrand','setCartData']),
...
<style>
  ...
  .btn-addtocart,.btn-addtofav
  {
    background-color: #17a2b8;
    margin: 2px 0;
    padding: 6px;
  }
  .btn-addtocart:hover,.btn-addtofav:hover
  {
